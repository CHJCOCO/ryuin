...
import { uploadMultipleFilesToServer, ServerUploadResult } from '@/lib/serverUpload';
import { isAllowedFileType, isFileSizeValid } from '@/lib/aws';
...
const ContactSection = () => {
  ...
  const handleFileUpload = (fileIndex: number, files: FileList | null) => {
    ...
    newUploadedFiles[fileIndex] = [file];
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (filesToUpload.length > 0) {
        ...
        const uploadResults = await uploadMultipleFilesToServer(filesToUpload);
        
        // 업로드 실패한 파일이 있는지 확인
        const failedUploads = uploadResults.filter((result: ServerUploadResult) => !result.success);
        ...
        fileUrls = uploadResults
          .filter((result: ServerUploadResult) => result.success && result.url)
          .map((result: ServerUploadResult) => result.url!);
              ...
              <div className="space-y-4">
                {[0, 1, 2].map((fileIndex) => (
                  <div key={fileIndex} className="relative">
                    {uploadedFiles[fileIndex].length === 0 ? (
                      <div>
                        <input
                          type="file"
                          id={`file-${fileIndex}`}
                      ...
                      <div className="flex items-center justify-between w-full px-0 py-4 border-0 border-b-2 border-gray-200">
                        <span 
                        ...
                        >
                          {uploadedFiles[fileIndex][0].name}
                        </span>
                        <button
                          type="button"
